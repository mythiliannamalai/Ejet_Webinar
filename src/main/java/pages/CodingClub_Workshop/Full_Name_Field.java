...
public class Full_Name_Field extends BaseClass {
    @FindBy(id="poptusername")
    WebElement txt_uname;
    @FindBy(id="poptuservalid")
    WebElement error;
    ...
    public boolean valid_FullName(String valid_name) throws InterruptedException, AWTException {
    ...
    public void user_FullName(String valid_name) throws InterruptedException, AWTException {
        if (!error.isDisplayed()) {
            JavascriptExecutor jsexe = (JavascriptExecutor) driver;
            jsexe.executeScript("window.scrollBy(0,500)");
        }
